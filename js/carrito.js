...
const productosEnCarrito = () => {
  if (carrito && carrito.length > 0) {
    ...
    carrito.forEach((producto) => {
      ...
      div.innerHTML = `
            <div class="col-2 border fs-5 text-center"><img class="imagenesCarrito " src="${
              producto.imagen
            }" alt="${producto.nombre}"></div>
            <div class="col-2 border fs-5 text-center">${producto.nombre}</div>
            <div class="col-2 border fs-5 text-center">${
              producto.cantidad
            }</div>
            <div class="col-2 border fs-5 text-center">$${
    ...
    .then((result) => {
      if (result.isConfirmed) {
        Swal.fire({
          ...
          icon: "success",
          title:
            "Tu compra se realizo con exito. Gracias por confiar en nostros!",
